...
function makeMocks() {
    const userRepository = { findByEmail: jest.fn() } as any as jest.Mocked<UserRepository>;
    const hashProvider = { compare: jest.fn() } as any as jest.Mocked<HashProvider>;
    const jwtService = { sign: jest.fn() } as any as jest.Mocked<JwtService>;
    const jwtPayloadService = { enrichPayload: jest.fn() } as any as jest.Mocked<JwtPayloadService>;
...
describe('LoginUseCase', () => {
    ...
    describe('happy path', () => {
        ...
        it('should call dependencies in correct order with correct args', async () => {
            ...
            // Assert
            expect(mocks.userRepository.findByEmail).toHaveBeenCalledWith(validInput.email);
            expect(mocks.hashProvider.compare).toHaveBeenCalledWith(validInput.password, user.passwordHash);
            expect(mocks.jwtPayloadService.enrichPayload).toHaveBeenCalledWith(user.id);
    ...
    describe('error cases', () => {
        ...
        it('should throw UnauthorizedException when user is INACTIVE', async () => {
            ...
            // Act & Assert
            await expect(sut.execute(validInput)).rejects.toThrow(UnauthorizedException);
            expect(mocks.hashProvider.compare).not.toHaveBeenCalled();
        });
