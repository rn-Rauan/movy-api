...
\paragraph{Auth Module}

Descrição: Autenticação, renovação de sessão e fluxos de entrada no sistema via JWT (Status: Implementado).

Principais casos de uso:
\begin{enumerate}
\item LoginUseCase: Valida credenciais, verifica usuário ativo e emite access token e refresh token.
\item RegisterUseCase: Auto-registro de novo usuário passageiro.
\item RefreshTokenUseCase: Renova credenciais e reidrata o payload do JWT com os dados atuais de organização e papel.
\item LogoutUseCase: Revoga o refresh token por identificador \texttt{jti}.
\item ForgotPasswordUseCase e ResetPasswordUseCase: Implementam recuperação de senha com token de uso único armazenado apenas como hash.
\item SendEmailVerificationUseCase e VerifyEmailUseCase: Implementam solicitação e confirmação de verificação de e-mail.
\item RegisterOrganizationWithAdminUseCase: Fluxo unificado de onboarding que cria usuário, organização e membership ADMIN atomicamente via \texttt{UnitOfWork}, com rollback automático pelo Prisma em caso de falha.
...
Endpoints: 9 rotas de autenticação em \texttt{/auth}: \texttt{login}, \texttt{register}, \texttt{refresh}, \texttt{logout}, \texttt{register-organization}, \texttt{setup-organization}, \texttt{forgot-password}, \texttt{reset-password} e \texttt{verify-email}.
...
Descrição: Gerenciamento do cadastro de usuários, incluindo operações de autosserviço e rotas administrativas de desenvolvimento (Status: Implementado).

Principais casos de uso:
\begin{enumerate}
\item CreateUserUseCase: Cadastro com validação de e-mail único e hash de senha Bcrypt.
\item FindAllUsersUseCase: Listagem paginada de todos os usuários.
\item FindAllActiveUsersUseCase: Listagem paginada apenas de usuários ativos.
\item FindUserByIdUseCase: Busca individual com validação 404.
\item UpdateUserUseCase: Atualização com revalidação dos dados alterados.
\item DisableUserUseCase: Soft-delete alterando status para INACTIVE.
\end{enumerate}

Endpoints: 8 rotas em \texttt{/users}. O autosserviço expõe \texttt{GET /users/me}, \texttt{PUT /users/me} e \texttt{DELETE /users/me}; as rotas de criação, listagem global, listagem de ativos, busca por ID e desativação por ID são restritas a usuários desenvolvedores via \texttt{DevGuard}.
