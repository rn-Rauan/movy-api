...
\item RF26 - Registro de Logs de auditoria (entidade modelada; persistência fora do escopo desta versão).
...
\paragraph{\textbf{AuditLog (Log de Auditoria --- modelado, não persistido)}}\mbox{}\\
Entidade \textbf{modelada no esquema, mas ainda não persistida} nesta versão: nenhum caso de uso escreve registros de auditoria. Mantém-se no modelo para documentar a intenção de design (o sistema é \textit{audit-ready}) e servir de ponto de extensão futuro. A implementação da persistência é tratada como dívida técnica declarada (ver Limitações). Quando ativada, registrará as ações críticas realizadas no sistema para conformidade e rastreabilidade, com os campos:
\begin{itemize}
    \item \textbf{id} -- Identificador único (UUID)
    \item \textbf{organizationId} -- FK: Organização
...
\item \textbf{Trilha de Auditoria}: A entidade \textit{AuditLog} (RF26) está modelada no esquema, mas sua persistência ainda não foi implementada --- nenhum caso de uso grava registros de auditoria. O modelo permanece como ponto de extensão (\textit{audit-ready}); a implementação exige um interceptor transversal sobre as mutações e uma política de retenção.
