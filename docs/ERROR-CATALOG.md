...
| termina em `_ALREADY_EXISTS` | `409 Conflict` |
| começa com `INVALID_` **ou** termina em `_BAD_REQUEST` | `400 Bad Request` |
| termina em `_FORBIDDEN` | `403 Forbidden` |
| termina em `_UNAUTHORIZED` | `401 Unauthorized` |
| **nenhuma das anteriores** | `500 Internal Server Error` |
...
## ⚠️ Discrepâncias conhecidas (intenção ≠ comportamento real)

Os códigos abaixo **não casam com nenhuma regra do filtro** e por isso retornam **HTTP 500**,
apesar do JSDoc/nome indicarem um 4xx. É um bug de convenção: o filtro nunca passou a tratar o
sufixo `_CONFLICT` (adotado nos módulos mais novos) nem os estados `INACTIVE_`/`EXPIRED_`.

| Código | HTTP real | HTTP pretendido (JSDoc/nome) | Causa |
|---|---|---|---|
| `BOOKING_ALREADY_EXISTS_CONFLICT` | 500 | 409 | sufixo `_CONFLICT` não tratado |
| `BOOKING_TRIP_INSTANCE_FULL_CONFLICT` | 500 | 409 | sufixo `_CONFLICT` não tratado |
| `DRIVER_ALREADY_EXISTS_CONFLICT` | 500 | 409 | sufixo `_CONFLICT` não tratado |
| `PLATE_ALREADY_IN_USE` | 500 | 409 | sufixo `_IN_USE` não tratado |
| `EXPIRED_CNH` | 500 | 400 | sem prefixo `INVALID_`/sufixo `_BAD_REQUEST` |
| `INACTIVE_USER` | 500 | 400 | idem |
| `INACTIVE_ORGANIZATION` | 500 | 400 | idem |
| `VEHICLE_INACTIVE` | 500 | 400 | idem |
| `TRIP_TEMPLATE_INACTIVE` | 500 | 400 | idem |

> **Correção sugerida (não aplicada):** adicionar regras `_CONFLICT → 409` e `_IN_USE → 409`
> ao filtro e renomear `EXPIRED_CNH`/`INACTIVE_*`/`*_INACTIVE` para terminarem em
> `_BAD_REQUEST` (ou adicionar regra própria). Como isso altera o contrato HTTP e pode
> impactar testes/cliente, foi deixado como decisão consciente.

Há ainda os erros de **falha de persistência** que retornam 500 **intencionalmente**
...
> Status na coluna **HTTP** é o **comportamento real atual** (já considerando as discrepâncias
> acima, marcadas com ⚠️).
...
| `INACTIVE_USER` | ⚠️ 500 | Operação sobre usuário já inativo (pretendido 400) |
...
| `INACTIVE_ORGANIZATION` | ⚠️ 500 | Operação sobre org inativa (pretendido 400) |
...
| `DRIVER_ALREADY_EXISTS_CONFLICT` | ⚠️ 500 | Motorista já existe (pretendido 409) |
...
| `EXPIRED_CNH` | ⚠️ 500 | CNH expirada (pretendido 400) |
...
| `PLATE_ALREADY_IN_USE` | ⚠️ 500 | Placa já em uso (pretendido 409) |
...
| `VEHICLE_INACTIVE` | ⚠️ 500 | Update de veículo inativo (pretendido 400) |
...
| `TRIP_TEMPLATE_INACTIVE` | ⚠️ 500 | Template inativo (pretendido 400) |
...
| `BOOKING_ALREADY_EXISTS_CONFLICT` | ⚠️ 500 | Reserva duplicada (pretendido 409) |
| `BOOKING_TRIP_INSTANCE_FULL_CONFLICT` | ⚠️ 500 | Instância sem vagas (pretendido 409) |
